...
async function callTodolist() {
  try {
    ...
    const addTasksToDom = todoList
      .map(function (task) {
        newArray =
          "<li id=" +
          task._id +
          '> <input type="checkbox" id="check"><span title="Double click to change task text">' +
          task.description +
          '</span><img class="delete" src="Yourtrash.png" /></li>';
        return newArray;
      })
      .join("");
